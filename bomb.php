...
<section class="product-single">
    <div class="circle-box">
        <img src="img/dhurata5.webp" alt="La Bomb Gift Box">
    </div>
    <div class="product-details">
        <h2 class="product-title">La Bomb Gift Box</h2>
        <p class="price">€85.00</p>
        <p class="description">Explosive flavors in every chocolate.</p>
        <h3>Ingredients:</h3>
        <ul>
            <li>Cocoa butter</li>
...
document.getElementById('add-to-cart').onclick = () => {
    let qty = parseInt(quantityInput.value);
    let existing = cart.find(item => item.name === "La Bomb Gift Box");
    if(existing) { 
        existing.qty += qty; 
    } else { 
        cart.push({ name: "La Bomb Gift Box", pricePerUnit: price, qty: qty, img: "dhurata5.webp" }); 
    }
...
function searchProduct(){
    ...
    if(searchValue.includes('bomb') || searchValue.includes('gift')){
        alert('Product found!');
    } else {
