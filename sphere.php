...
<nav class="navbar">
    ...
    <div class="nav-right">
        ...
        <a href="cart.php" class="cart-icon" id="cartIcon"><img src="https://img.icons8.com/ios/50/shopping-cart--v1.png" alt="Cart"><span id="cartCount" class="cart-count">0</span></a>
...
<section class="product-single">
    ...
    <div class="product-details">
        <h2 class="product-title">Sphère Gift Box</h2>
        ...
        <p class="total-price">Total: €26.00</p>
...
document.getElementById('add-to-cart').onclick = () => {
    let qty = parseInt(quantityInput.value);
    let existing = cart.find(item => item.name === "Sphère Gift Box");
    if(existing) { existing.qty += qty; } 
    else { cart.push({ name: "Sphère Gift Box", pricePerUnit: price, qty: qty, img: "dhurata.webp" }); }
...
function updateMiniCart(){
    ...
    cart.forEach(item => {
        ...
        li.innerHTML = `<img src="img/${item.img}" alt="${item.name}"><span>${item.name} €${item.pricePerUnit.toFixed(2)} x ${item.qty} = €${(item.pricePerUnit*item.qty).toFixed(2)}</span>`;
        cartItems.appendChild(li);
    });
    document.getElementById('totalPriceMini').textContent = `Total: €${total.toFixed(2)}`;
